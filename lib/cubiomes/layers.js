...
function mcFirstInt(s, mod) {
    const sBig = BigInt.asIntN(64, BigInt(s));
...
function mcFirstIsZero(s, mod) {
    const sBig = BigInt.asIntN(64, BigInt(s));
    return Number((sBig >> 24n) % BigInt(mod)) === 0;
}

function getLayerSalt(salt) {
    salt = BigInt(salt);
    let ls = (salt * (salt * 6364136223846793005n + 1442695040888963407n) + salt) & 0xFFFFFFFFFFFFFFFFn;
    ls = (ls * (ls * 6364136223846793005n + 1442695040888963407n) + salt) & 0xFFFFFFFFFFFFFFFFn;
    ls = (ls * (ls * 6364136223846793005n + 1442695040888963407n) + salt) & 0xFFFFFFFFFFFFFFFFn;
    return ls;
}
...
function mapZoom(l, out, x, z, w, h) {
    ...
    const readParent = (idx) => idx < parentBuf.length ? parentBuf[idx] : 0;

    for (let j = 0; j < pH; j++) {
        let idx = j * 2 * newW;
        let v00 = readParent(j * pW), v01 = readParent((j + 1) * pW);
        for (let i = 0; i < pW; i++) {
            const v10 = readParent((i + 1) + j * pW), v11 = readParent((i + 1) + (j + 1) * pW);
            if (v00 === v01 && v00 === v10 && v00 === v11) {
                buf[idx] = buf[idx + 1] = buf[idx + newW] = buf[idx + newW + 1] = v00;
                idx += 2; v00 = v10; v01 = v11; continue;
            }
            const chunkX = BigInt((i + pX) * 2), chunkZ = BigInt((j + pZ) * 2);
            let cs = BigInt(ss);
            cs = (cs + chunkX) & 0xFFFFFFFFn;
            cs = (cs * (cs * 1284865837n + 4150755663n)) & 0xFFFFFFFFn;
            cs = (cs + chunkZ) & 0xFFFFFFFFn;
            cs = (cs * (cs * 1284865837n + 4150755663n)) & 0xFFFFFFFFn;
            cs = (cs + chunkX) & 0xFFFFFFFFn;
            cs = (cs * (cs * 1284865837n + 4150755663n)) & 0xFFFFFFFFn;
            cs = (cs + chunkZ) & 0xFFFFFFFFn;
            ...
            buf[idx] = v00;
            buf[idx + newW] = ((cs >> 24n) & 1n) ? v01 : v00;
            idx++;
            cs = (cs * (cs * 1284865837n + 4150755663n)) & 0xFFFFFFFFn;
            cs = (cs + BigInt(st)) & 0xFFFFFFFFn;
            buf[idx] = ((cs >> 24n) & 1n) ? v10 : v00;
...
function mapLandB18(l, out, x, z, w, h) {
    ...
    for (let j = 0; j < h; j++) {
        ...
        let v00 = parentBuf[vz0Off + 0], vt0 = parentBuf[vz0Off + 1];
        let v02 = parentBuf[vz2Off + 0], vt2 = parentBuf[vz2Off + 1];

        for (let i = 0; i < w; i++) {
            const v11 = parentBuf[vz1Off + i + 1];
            const v20 = parentBuf[vz0Off + i + 2];
            const v22 = parentBuf[vz2Off + i + 2];
            let v = v11;

            if (v11 === 0 && (v00 !== 0 || v02 !== 0 || v20 !== 0 || v22 !== 0)) {
                const cs = getChunkSeed(ss, i + x, j + z);
                v = (mcFirstInt(cs, 3) / 2) | 0;
            } else if (v11 === 1 && (v00 !== 1 || v02 !== 1 || v20 !== 1 || v22 !== 1)) {
                const cs = getChunkSeed(ss, i + x, j + z);
                v = 1 - ((mcFirstInt(cs, 5) / 4) | 0);
            }
            out[i + j * w] = v;
            v00 = vt0; vt0 = v20;
            v02 = vt2; vt2 = v22;
...
function mapNoise(l, out, x, z, w, h) {
    ...
    for (let j = 0; j < h; j++) {
        for (let i = 0; i < w; i++) {
            const idx = i + j * w;
            if ((out[idx] | 0) > 0) {
                const cs = getChunkSeed(ss, (i + x) | 0, (j + z) | 0);
                out[idx] = (mcFirstInt(cs, mod) + 2) | 0;
            } else {
                out[idx] = 0;
            }
        }
...
export function setupLayerStack(mc, largeBiomes = false) {
    const layers = {};
    const map_land = mc === MC_B1_8 ? mapLandB18 : (mc <= MC_1_6) ? mapLand16 : mapLand;
    let p;
    ...
    if (mc === MC_B1_8) {
        ...
        // Note: Shore layer at scale 32 for Beta 1.8
        ...
        let r = layers.zoom128River = new Layer(mapZoom, 128, 1000, layers.riverInit256);
        r = layers.zoom64River = new Layer(mapZoom, 64, 1001, r);
        r = layers.zoom32River = new Layer(mapZoom, 32, 1002, r);
        r = layers.zoom16River = new Layer(mapZoom, 16, 1003, r);
        r = layers.zoom8River = new Layer(mapZoom, 8, 1004, r);
        r = layers.zoom4River = new Layer(mapZoom, 4, 1005, r);
        r = layers.river4 = new Layer(mapRiver, 4, 1, r);
        r = layers.smooth4River = new Layer(mapSmooth, 4, 1000, r);
        const smooth4River = r;
...
export class LegacyBiomeGenerator {
    ...
    getBiome(x, z) {
        if (this.version === MC_B1_7) {
            return this.snb.getBiome(Math.floor(x), Math.floor(z), 1);
        }
        const bx = Math.floor(x) / 4, bz = Math.floor(z) / 4;
        const out = new Int32Array(1);
        const ibx = Math.floor(bx), ibz = Math.floor(bz);
        this.entry.getMap(this.entry, out, ibx, ibz, 1, 1);
        return out[0];
    }
    getArea(x, z, w, h, scale = 4) {
        if (this.version === MC_B1_7) {
            ...
            for (let j = 0; j < sh; j++) {
                for (let i = 0; i < sw; i++) {
                    out[i + j * sw] = this.getBiome((sx + i) * scale, (sz + j) * scale);
                }
            }

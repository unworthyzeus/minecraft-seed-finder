...
export const MC_B1_7 = 10;
export const MC_B1_8 = 11;
export const MC_1_0 = 12;
export const MC_1_1 = 13;
export const MC_1_2 = 14;
export const MC_1_3 = 15;
export const MC_1_4 = 16;
export const MC_1_5 = 17;
export const MC_1_6 = 18;
export const MC_1_7 = 19;
export const MC_1_8 = 20;
export const MC_1_9 = 21;
export const MC_1_10 = 22;
export const MC_1_11 = 23;
export const MC_1_12 = 24;
export const MC_1_13 = 25;
export const MC_1_14 = 26;
export const MC_1_15 = 27;
export const MC_1_16 = 29;
export const MC_1_17 = 30;
...
function getCategory(mc, id) {
    switch (id) {
        ...
        case wooded_badlands_plateau: case badlands_plateau: return (mc <= 15) ? badlands : badlands_plateau;
    ...
    for (let j = 0; j < h; j++) {
        for (let i = 0; i < w; i++) {
            ...
            if (v11 !== 0 || (v00 === 0 && v20 === 0 && v02 === 0 && v22 === 0)) {
                ...
                if (v02 !== ocean) {
                    if (++inc === 1 || (inc === 2 && mcFirstIsZero(cs, 2)) || mcFirstIsZero(cs, 3)) v = v02;
                    cs = mcStepSeed(cs, st);
                }
                if (v22 !== ocean) {
                    inc++;
                    if (inc === 1 || (inc === 2 && mcFirstIsZero(cs, 2)) ||
                        (inc === 3 && mcFirstIsZero(cs, 3)) || mcFirstIsZero(cs, 4)) v = v22;
                    cs = mcStepSeed(cs, st);
                }
    ...
    for (let j = 0; j < h; j++) {
        for (let i = 0; i < w; i++) {
            ...
            if (v11 === 0 && (v00 !== 0 || v02 !== 0 || v20 !== 0 || v22 !== 0)) {
                // Should return 0 or 1. If mapLand logic where land is 1.
                // In B1.8, is land 1?
                // C code: v = mcFirstInt(cs, 3) / 2; -> 0,0,1 -> 1/3 chance of 1.
                const cs = getChunkSeed(ss, i + x, j + z);
                v = Math.floor(mcFirstInt(cs, 3) / 2);
            } else if (v11 === 1 && (v00 !== 1 || v02 !== 1 || v20 !== 1 || v22 !== 1)) {
                // C code: v = 1 - mcFirstInt(cs, 5) / 4; -> 0,1,2,3 -> 0. 4 -> 1. 
                // So (0,1,2,3)/4 = 0. 4/4 = 1.
                // So 4/5 chance of 1, 1/5 chance of 0.
                const cs = getChunkSeed(ss, i + x, j + z);
                v = 1 - Math.floor(mcFirstInt(cs, 5) / 4);
            }
            out[i + j * w] = v;
        }
    }
...
function mapBiome(l, out, x, z, w, h) {
    ...
    for (let j = 0; j < h; j++) {
        for (let i = 0; i < w; i++) {
            const idx = i + j * w;
            let id = out[idx];
            const hasHighBit = (id & 0xf00); id &= ~0xf00;
            if (isOceanic(id) || id === mushroom_fields) continue;
            const cs = getChunkSeed(ss, i + x, j + z);
            let v;
            switch (id) {
                case Warm: v = hasHighBit ? (mcFirstIsZero(cs, 3) ? badlands_plateau : wooded_badlands_plateau) : warmBiomes[mcFirstInt(cs, 6)]; break;
                case Lush: v = hasHighBit ? jungle : lushBiomes[mcFirstInt(cs, 6)]; break;
                case Cold: v = hasHighBit ? giant_tree_taiga : coldBiomes[mcFirstInt(cs, 4)]; break;
                case Freezing: v = snowBiomes[mcFirstInt(cs, 4)]; break;
                default: v = mushroom_fields;
            }
            out[idx] = v;
        }
    }
...
function mapRiverMix(l, out, x, z, w, h) {
    ...
    for (let idx = 0; idx < w * h; idx++) {
        const biome = out[idx], r = riv[idx];
        if (isOceanic(biome)) continue;
        if (r === river) {
            if (biome === snowy_tundra) out[idx] = frozen_river;
            else if (biome === mushroom_fields || biome === mushroom_field_shore) out[idx] = mushroom_field_shore;
            else out[idx] = river;
        }
    }
    return 0;
    ...
    for (let j = 0; j < h; j++) {
        for (let i = 0; i < w; i++) {
            ...
            if (v11 === mushroom_fields) {
                out[idx] = [v10, v21, v01, v12].some(v => v === ocean) ? mushroom_field_shore : v11;
            } else if (mc >= 7) {
                ...
                if (v11 === mountains) {
                    ...
                    if (isAny4Oceanic(v10, v21, v01, v12)) v11 = beach;
                }
                out[idx] = v11;
            }
        }
            ...
            if (bn === 1 && b11 >= 2 && !isShallowOcean(a11)) {
                ...
                if (bn === 0 || mcFirstIsZero(cs, 3)) {
                    ...
                    switch (a11) {
                        ...
                        case plains: hillID = (mc <= 6) ? forest : (mcFirstIsZero(mcStepSeed(cs, st), 3) ? wooded_hills : forest); break;
                        ...
                        case ocean: if (mc >= 7) hillID = deep_ocean; break;
                        case mountains: if (mc >= 7) hillID = wooded_mountains; break;
                    ...
                    if (hillID !== a11) {
                        ...
                        out[idx] = (equals >= 3 + (mc <= 6 ? 1 : 0)) ? hillID : a11;
                    } else out[idx] = a11;
                } else out[idx] = a11;
            }
        }
...
export function setupLayerStack(mc, largeBiomes = false) {
    ...
    if (mc === MC_B1_8) {
        layers.land2048 = new Layer(mapLandB18, 2048, 1, layers.zoom2048);
        layers.zoom1024 = new Layer(mapZoom, 1024, 2001, layers.land2048);
        layers.land1024a = new Layer(mapLandB18, 1024, 2, layers.zoom1024);
        layers.zoom512 = new Layer(mapZoom, 512, 2003, layers.land1024a);
        layers.land512 = new Layer(mapLandB18, 512, 3, layers.zoom512);

        layers.zoom256 = new Layer(mapZoom, 256, 2004, layers.land512);
        layers.land256 = new Layer(mapLandB18, 256, 3, layers.zoom256);
        layers.biome256 = new Layer(mapBiome, 256, 200, layers.land256);

        layers.zoom128 = new Layer(mapZoom, 128, 1000, layers.biome256);
        layers.zoom64 = new Layer(mapZoom, 64, 1001, layers.zoom128);
        layers.riverInit256 = new Layer((l, o, x, z, w, h) => {
            if (l.p.getMap(l.p, o, x, z, w, h) !== 0) return -1;
            for (let idx = 0; idx < w * h; idx++) {
                if (o[idx] > 0) {
                    const cs = getChunkSeed(l.startSeed, (idx % w) + x, Math.floor(idx / w) + z);
                    o[idx] = mcFirstInt(cs, 299999) + 2;
                } else o[idx] = 0;
            }
            return 0;
        }, 256, 100, layers.land256);
    } else if (mc <= MC_1_6) {
        layers.land2048 = new Layer(mapLand16, 2048, 1, layers.zoom2048);
        layers.zoom1024 = new Layer(mapZoom, 1024, 2001, layers.land2048);
        layers.land1024a = new Layer(mapLand16, 1024, 2, layers.zoom1024);
        layers.snow1024 = new Layer(mapSnow16, 1024, 2, layers.land1024a);
        layers.zoom512 = new Layer(mapZoom, 512, 2002, layers.snow1024);
        layers.land512 = new Layer(mapLand16, 512, 3, layers.zoom512);
        layers.zoom256 = new Layer(mapZoom, 256, 2003, layers.land512);
        layers.land256 = new Layer(mapLand16, 256, 4, layers.zoom256);
        ...
        layers.land2048 = new Layer(mapLand, 2048, 1, layers.zoom2048);
        layers.zoom1024 = new Layer(mapZoom, 1024, 2001, layers.land2048);
        ...
        if (mc >= MC_1_7) {
            ...
            layers.cool1024 = new Layer(mapCool, 1024, 2, layers.land1024d);
            layers.heat1024 = new Layer(mapHeat, 1024, 2, layers.cool1024);
        ...
        layers.zoom512 = new Layer(mapZoom, 512, 2002, layers.parent512);
        if (mc <= MC_1_6) {
            layers.land512 = new Layer(mapLand, 512, 4, layers.zoom512);
            layers.parentZoom256 = layers.land512;
        } else layers.parentZoom256 = layers.zoom512;
        layers.zoom256 = new Layer(mapZoom, 256, 2003, layers.parentZoom256);
        layers.land256 = new Layer(mapLand, 256, 4, layers.zoom256);
        layers.mushroom256 = new Layer(mapMushroom, 256, 5, layers.land256);
        if (mc >= MC_1_7) {
            layers.deepOcean256 = new Layer(mapDeepOcean, 256, 4, layers.mushroom256);
            layers.biome256 = new Layer(mapBiome, 256, 200, layers.deepOcean256);
        } else layers.biome256 = new Layer(mapBiome, 256, 200, layers.mushroom256);

        layers.zoom128 = new Layer(mapZoom, 128, 1000, layers.biome256);
        layers.zoom64 = new Layer(mapZoom, 64, 1001, layers.zoom128);
        layers.biomeEdge64 = new Layer(mapBiomeEdge, 64, 1000, layers.zoom64);
        layers.riverInit256 = new Layer((l, o, x, z, w, h) => {
            if (l.p.getMap(l.p, o, x, z, w, h) !== 0) return -1;
            for (let idx = 0; idx < w * h; idx++) {
                if (o[idx] > 0) {
            ...
            return 0;
        }, 256, 100, (mc >= MC_1_7) ? layers.deepOcean256 : layers.mushroom256);
    }

    const hS1 = (mc <= MC_1_0) ? 0 : 1000, hS2 = (mc <= MC_1_0) ? 0 : 1001;
    ...
    if (!hillsParent) {
        console.error(`setupLayerStack: hillsParent is null! mc=${mc} (MC_1_6=${MC_1_6})`);
        console.error(`zoom64=${layers.zoom64} biomeEdge64=${layers.biomeEdge64}`);
    }

    layers.hills64 = new Layer(mapHills, 64, 1000, hillsParent, layers.zoom64Hills);
    layers.sunflower64 = new Layer(mapSunflower, 64, 1001, layers.hills64);
    layers.zoom32 = new Layer(mapZoom, 32, 1000, layers.sunflower64);
    layers.land32 = new Layer(mapLand, 32, 3, layers.zoom32);
    layers.zoom16 = new Layer(mapZoom, 16, 1001, layers.land32);
    layers.shore16 = new Layer(mapShore, 16, 1000, layers.zoom16);
    layers.zoom8 = new Layer(mapZoom, 8, 1002, layers.shore16);
    layers.zoom4 = new Layer(mapZoom, 4, 1003, layers.zoom8);
    layers.smooth4 = new Layer(mapSmooth, 4, 1000, layers.zoom4);
    layers.zoom128River = new Layer(mapZoom, 128, 1000, layers.riverInit256);
    layers.zoom64River = new Layer(mapZoom, 64, 1001, layers.zoom128River);
    layers.zoom32River = new Layer(mapZoom, 32, 1000, layers.zoom64River);
    layers.zoom16River = new Layer(mapZoom, 16, 1001, layers.zoom32River);
    layers.zoom8River = new Layer(mapZoom, 8, 1002, layers.zoom16River);
    layers.zoom4River = new Layer(mapZoom, 4, 1003, layers.zoom8River);
    layers.river4 = new Layer(mapRiver, 4, 1, layers.zoom4River);
    ...
    if (mc >= MC_1_13) {
        ...
        layers.oceanMix4 = new Layer(mapOceanMix, 4, 100, layers.riverMix4, layers.zoom4Ocean);
    }
...
export class LegacyBiomeGenerator {
    constructor(seed, version = 13) {
        ...
        this.entry = entry;
        this.entry.setWorldSeed(this.seed);
    }
    getBiome(x, z) {

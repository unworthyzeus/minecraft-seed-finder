...
const BEDROCK_PLACEMENT_SOURCE = 'mcbe-structure-finder';
const BEDROCK_FALLBACK_SOURCE = 'java-parity-fallback';

const BEDROCK_STRUCTURE_CONFIGS = new Map([
    [STRUCTURE_TYPES.Village, { spacing: 27, spawnRange: 17, salt: 10387312, num: 4, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Desert_Pyramid, { spacing: 32, spawnRange: 24, salt: 14357617, num: 2, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Jungle_Temple, { spacing: 32, spawnRange: 24, salt: 14357617, num: 2, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Swamp_Hut, { spacing: 32, spawnRange: 24, salt: 14357617, num: 2, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Igloo, { spacing: 32, spawnRange: 24, salt: 14357617, num: 2, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Monument, { spacing: 32, spawnRange: 27, salt: 10387313, num: 4, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Mansion, { spacing: 80, spawnRange: 60, salt: 10387319, num: 4, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Outpost, { spacing: 80, spawnRange: 56, salt: 165745296, num: 4, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Shipwreck, { spacing: 10, spawnRange: 5, salt: 1, num: 4, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Ocean_Ruin, { spacing: 12, spawnRange: 5, salt: 14357621, num: 4, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Treasure, { spacing: 4, spawnRange: 2, salt: 16842397, num: 4, source: BEDROCK_PLACEMENT_SOURCE }],
    [STRUCTURE_TYPES.Ruined_Portal, { spacing: 40, spawnRange: 25, salt: 40552231, num: 4, source: BEDROCK_PLACEMENT_SOURCE }],
]);
...
export function getBedrockStructureConfig(type) {
    return BEDROCK_STRUCTURE_CONFIGS.get(type) || null;
}
...
export function getBedrockStructurePos(type, seedInput, areaX, areaZ) {
    const config = getBedrockStructureConfig(type);
...
function bedrockAreaContainsOnly(generator, centerX, centerZ, radius, allowedBiomes) {
    if (!generator || typeof generator.getBiome !== 'function') return { viable: false, biome: null };
    let firstBiome = null;
    ...
    for (let cellZ = minCellZ; cellZ <= maxCellZ; cellZ++) {
        for (let cellX = minCellX; cellX <= maxCellX; cellX++) {
            const biome = generator.getBiome(cellX * 4, cellZ * 4);
...
export function checkBedrockStructureBiome(type, generator, x, z) {
    ...
    switch (type) {
        ...
        default:
            return {
                viable: true,
                biome: generator && typeof generator.getBiome === 'function' ? generator.getBiome(centerX, centerZ) : null,
            };
    }
}
...
function getCandidateStatus({ edition, mc, type, biomeViable, placementSource }) {
    ...
    if (edition === 'bedrock') {
        ...
        return {
            ...
            reason: 'Bedrock placement uses a JavaScript port of MCBE structure-finder rules with biome checks where available; final Bedrock terrain/feature generation still needs in-game or fixture verification.',
            requiresTerrainCheck: false,
        };
    }
    ...
    return {
        status: 'placement-candidate',
        label: 'Placement candidate',
        reason: 'Java region placement and biome rules match; final generated-world confirmation is still required.',
        requiresTerrainCheck,
    };
}
...
export function generateStructureCandidates({
    ...
    for (const [key, info] of entries) {
        const bedrockConfig = edition === 'bedrock' ? getBedrockStructureConfig(info.type) : null;
        ...
        for (let rx = minRX; rx <= maxRX; rx++) {
            for (let rz = minRZ; rz <= maxRZ; rz++) {
                const pos = bedrockConfig
                    ? getBedrockStructurePos(info.type, seed, rx, rz)

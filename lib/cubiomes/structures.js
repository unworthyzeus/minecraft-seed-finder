...
function getCandidateStatus({ edition, mc, type, biomeViable }) {
    if (!biomeViable) {
        return {
    ...
    if (edition === 'bedrock') {
        return {
            status: 'bedrock-candidate',
            label: 'Bedrock candidate',
            reason: 'Bedrock biome parity is checked, but Bedrock structure generation is not fully ground-truthed here.',
            requiresTerrainCheck: false,
        };
    }

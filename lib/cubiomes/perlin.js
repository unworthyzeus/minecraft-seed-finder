...
class PerlinNoise {
    ...
    sampleBeta17Terrain(v, d1, d3, yLacAmp) {
        ...
        const p = this.terrainBytes || this.p, ii1 = i1 & 0xFF, ii3 = i3 & 0xFF;
        ...
        for (let yi = yic; yi <= 8; yi++) {
            ...
            if (yi === 0 || ii2 !== genFlag) {
                genFlag = ii2;
                const a1 = p[ii1] + ii2, b1 = p[ii1 + 1] + ii2;
                const a2 = p[a1] + ii3, a3 = p[a1 + 1] + ii3;
                const b2 = p[b1] + ii3, b3 = p[b1 + 1] + ii3;
...
class SurfaceNoiseBeta {
    constructor(seed) {
        ...
        attachBetaTerrainMemory([
            ...this.octmin.octaves,
            ...this.octmax.octaves,
            ...this.octmain.octaves,
            ...this.octcontA.octaves,
            ...this.octcontB.octaves,
        ]);
        ...
        this.detail = new OctaveNoise(createSeed(seed * 543321n), 2, 0.25 / 1.5, 10.0 / 17.0, 0.55, 2.0);
    }

    sampleClimate(blockX, blockZ) {
        const tRaw = this.temperature.sampleModern(blockX, blockZ);
        const hRaw = this.humidity.sampleModern(blockX, blockZ);
        const fRaw = this.detail.sampleModern(blockX, blockZ);

        const f = fRaw * 1.1 + 0.5;
        let t = (tRaw * 0.15 + 0.7) * 0.99 + f * 0.01;
        t = 1.0 - (1.0 - t) * (1.0 - t);
        if (t < 0) t = 0; else if (t > 1) t = 1;

        let h = (hRaw * 0.15 + 0.5) * 0.998 + f * 0.002;
        if (h < 0) h = 0; else if (h > 1) h = 1;

        return [t, h];
    }

    sampleColumn(cellX, cellZ, lacmin = 0) {
        const contA = this.octcontA.samplePerlin(cellX, 0, cellZ, 0, 0);
        const contB = this.octcontB.samplePerlin(cellX, 0, cellZ, 0, 0);
        const minS = [0, 0], maxS = [0, 0], mainS = [0, 0];
        this.octmin.sampleBeta17Terrain(minS, cellX, cellZ, 0, lacmin);
        this.octmax.sampleBeta17Terrain(maxS, cellX, cellZ, 0, lacmin);
        this.octmain.sampleBeta17Terrain(mainS, cellX, cellZ, 1, lacmin);

        return { contA, contB, minS, maxS, mainS };
    }

    processColumn(column, climate) {
        let humi = 1.0 - climate[0] * climate[1];
        humi *= humi;
        humi *= humi;
        humi = 1.0 - humi;

        let cA = (column.contA + 256.0) / 512.0 * humi;
        if (cA > 1.0) cA = 1.0;

        let cB = column.contB / 8000.0;
        if (cB < 0) cB = -cB * 0.3;
        cB = cB * 3.0 - 2.0;

        if (cB < 0) {
            cB /= 2.0;
            cB = (cB < -1.0) ? -1.0 / 1.4 / 2.0 : cB / 1.4 / 2.0;
            cA = 0;
        } else {
            cB = (cB > 1.0) ? 1.0 / 8.0 : cB / 8.0;
        }
        ...
        const out = [0, 0];
        for (let i = 0; i < 2; i++) {
            let procCont = ((i + 7 - cB) * 12.0) / cA;
            procCont = (procCont < 0) ? procCont * 4.0 : procCont;

            const low = column.minS[i] / 512.0;
            const high = column.maxS[i] / 512.0;
            const selector = (column.mainS[i] / 10.0 + 1.0) / 2.0;
            const chosen = (selector < 0.0) ? low : (selector > 1.0) ? high : low + (high - low) * selector;
            out[i] = chosen - procCont;
        }

        return out;
    }

    getBiomeAtBlock(blockX, blockZ) {
        const climate = this.sampleClimate(blockX, blockZ);
        let id = getOldBetaBiome(climate[0], climate[1]);

        const cx = Math.floor(blockX / 4);
        const cz = Math.floor(blockZ / 4);
        const csx = (cx * 4) & ~15;
        const csz = (cz * 4) & ~15;
        const ci = cx & 3;
        const cj = cz & 3;
        const off = [1, 4, 7, 10, 13];

        const processAt = (cellX, cellZ, sampleX, sampleZ) => {
            const sampleClimate = this.sampleClimate(sampleX, sampleZ);
            return this.processColumn(this.sampleColumn(cellX, cellZ, 0), sampleClimate);
        };

        const cols00 = processAt(cx, cz, csx + off[ci], csz + off[cj]);
        const cols10 = processAt(cx + 1, cz, csx + off[ci + 1], csz + off[cj]);
        const cols01 = processAt(cx, cz + 1, csx + off[ci], csz + off[cj + 1]);
        const cols11 = processAt(cx + 1, cz + 1, csx + off[ci + 1], csz + off[cj + 1]);

        const dx = (blockX & 3) * 0.25;
        const dz = (blockZ & 3) * 0.25;
        const dy = 7 / 8;
        const b00 = cols00[0] + (cols00[1] - cols00[0]) * dy;
        const b01 = cols10[0] + (cols10[1] - cols10[0]) * dy;
        const b10 = cols01[0] + (cols01[1] - cols01[0]) * dy;
        const b11 = cols11[0] + (cols11[1] - cols11[0]) * dy;
    ...
    getBiome(blockX, blockZ, scale = 4) {
        if (scale < 4) return this.getBiomeAtBlock(blockX, blockZ);

        const climate = this.sampleClimate(blockX, blockZ);
        let id = getOldBetaBiome(climate[0], climate[1]);

        const column = this.sampleColumn(blockX * 0.25, blockZ * 0.25, 4.0 / scale);
        const sea = this.processColumn(column, climate);
        if (sea[0] * 0.125 + sea[1] * 0.875 <= 0) {
            id = climate[0] < 0.5 ? frozen_ocean : ocean;
        }
...
class BiomeNoiseBeta {
    ...
    sample(x, z) {
        return this.snb.sampleClimate(x, z);
    }
}

function attachBetaTerrainMemory(octaves) {
    const structSize = 320;
    const bytes = new Uint8Array(structSize * octaves.length + 512);
    ...
    for (let i = 0; i < octaves.length; i++) {
        ...
        bytes.set(oct.p, base);
        bytes[base + 257] = oct.h2;

        view.setFloat64(base + 264, oct.a, true);
        view.setFloat64(base + 272, oct.b, true);
        view.setFloat64(base + 280, oct.c, true);
        view.setFloat64(base + 288, oct.amplitude, true);
        view.setFloat64(base + 296, oct.lacunarity, true);
        view.setFloat64(base + 304, oct.d2, true);
        view.setFloat64(base + 312, oct.t2, true);
    }

    for (let i = 0; i < octaves.length; i++) {
        const base = i * structSize;
        octaves[i].terrainBytes = bytes.subarray(base, base + 512);
    }
}

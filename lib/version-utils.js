...
export const BOTH_WORLDGEN_VERSION_OPTIONS = [
    option('26.1.2|26.13', 'Java 26.1.2 + Bedrock 26.13 (current)'),
    option('26.1|26.10', 'Java 26.1 + Bedrock 26.10'),
    option('1.21.11|1.21.132', 'Java 1.21.11 + Bedrock 1.21.132'),
    option('1.21|1.21.0', 'Java 1.21 + Bedrock 1.21.0'),
    option('1.20|1.20.0', 'Java 1.20 + Bedrock 1.20.0'),
    option('1.19|1.19.0', 'Java 1.19 + Bedrock 1.19.0'),
    option('1.18|1.18.0', 'Java 1.18 + Bedrock 1.18.0'),
];

const FILTER_COMMON = [
    option('all', 'All Versions'),
    option('26.', '26.x Current'),
    option('1.21', '1.21 family'),
    option('1.20', '1.20 family'),
    option('1.19', '1.19 family'),
    option('1.18', '1.18 family'),
];

export const VERSION_FILTER_OPTIONS_BY_EDITION = {
    all: [
        ...FILTER_COMMON,
        option('1.17', '1.17 family'),
        option('1.16', '1.16 family'),
        option('1.15', '1.15 family'),
        option('1.14', '1.14 family'),
        option('1.13', '1.13 family'),
        option('1.12', '1.12 family'),
    ...
    both: [
        option('all', 'All Dual-Edition Seeds'),
        option('both_current', 'Current Pair: Java 26.1.2 + Bedrock 26.13'),
        option('26.', 'Both 26.x'),
        option('1.21', 'Both 1.21 family'),
        option('1.20', 'Both 1.20 family'),
        option('1.19', 'Both 1.19 family'),
        option('1.18', 'Both 1.18 family'),
        option('1.17', 'Both 1.17 family'),
        option('1.16', 'Both 1.16 family'),
    ],
};

export function getVersionSelectOptions(edition = 'java') {
    if (edition === 'bedrock') return BEDROCK_WORLDGEN_VERSION_OPTIONS;
    if (edition === 'both') return BOTH_WORLDGEN_VERSION_OPTIONS;
    return JAVA_WORLDGEN_VERSION_OPTIONS;
}

export function getVersionFilterOptions(edition = 'all') {
    return VERSION_FILTER_OPTIONS_BY_EDITION[edition] || VERSION_FILTER_OPTIONS_BY_EDITION.all;
}
...
export function getDefaultVersionForEdition(edition = 'java') {
    ...
    if (edition === 'both') return `${CURRENT_MINECRAFT_VERSIONS.java.version}|${CURRENT_MINECRAFT_VERSIONS.bedrock.version}`;
...
export function splitEditionVersion(edition, value) {
    if (edition === 'both') {
        const [javaVersion, bedrockVersion] = String(value || getDefaultVersionForEdition('both')).split('|');
        return {
            java: javaVersion || CURRENT_MINECRAFT_VERSIONS.java.version,
            bedrock: bedrockVersion || CURRENT_MINECRAFT_VERSIONS.bedrock.version,
        };
    }
...
export function formatEditionVersion(edition, value) {
    ...
    const versions = splitEditionVersion('both', value);
    return `Java ${versions.java} / Bedrock ${versions.bedrock}`;
}
...
export function seedMatchesVersionFilter(seed, editionFilter, versionFilter) {
    ...
    if (versionFilter === 'both_current') {
        return javaVersion === CURRENT_MINECRAFT_VERSIONS.java.version &&
            bedrockVersion === CURRENT_MINECRAFT_VERSIONS.bedrock.version;
    }

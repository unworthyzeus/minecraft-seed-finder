...
function determineCategory(title, flair) {
    ...
    // Village strict categorization
    if (t.includes('village') && (t.includes('zombie') || t.includes('abandoned') || t.includes('double') || t.includes('glitch') || t.includes('spider'))) return 'village_anomaly';
    if (t.includes('village')) return 'rare_biome'; // Normal villages

    if (t.includes('outpost') && (t.includes('village') || t.includes('inside') || t.includes('middle'))) return 'structure_combo';
    if (t.includes('outpost')) return 'rare_biome';
    ...
    if (t.includes('mineshaft') && (t.includes('surface') || t.includes('exposed') || t.includes('glitch') || t.includes('high') || t.includes('floating'))) return 'mineshaft_anomaly';
    if (t.includes('mineshaft') && (t.includes('ancient') || t.includes('trial') || t.includes('colliding') || t.includes('overlap'))) return 'structure_combo';
    if (t.includes('mineshaft')) return 'rare_biome'; // Normal mineshafts are just biomes/structures
...
if (fs.existsSync(dataDir)) {
    const files = fs.readdirSync(dataDir).filter(f =>
        (f.startsWith('reddit-seeds-') || f === 'theoretical-anomalies.json') && f.endsWith('.json')
    );
        ...
        try {
            ...
            if (Array.isArray(redditData)) {
                ...
                redditData.forEach(item => {
                    // Re-classify based on newest logic during build (unless it's theoretical-anomalies which are manual)
                    const isTheoretical = file === 'theoretical-anomalies.json';
                    const newCategory = isTheoretical ? item.category : determineCategory(item.title, "");

                    seeds.push({
                        ...item,
                        category: newCategory, // Override category with corrected logic
                        confidence: item.confidence || (isTheoretical ? 1.0 : 0.9),
                        isGenerated: false
                    });
                });
            }
...
// Process user-submitted seeds after the generated source lists so their
// stable IDs and submission metadata are preserved by every rebuild.
const submittedPath = path.join(__dirname, '../data/submitted-seeds.json');
if (fs.existsSync(submittedPath)) {
    const submittedData = JSON.parse(fs.readFileSync(submittedPath, 'utf8'));
    if (submittedData.seeds && Array.isArray(submittedData.seeds)) {
        console.log(`Found ${submittedData.seeds.length} submitted seeds`);
        submittedData.seeds.forEach(item => {
            seeds.push({
                ...item,
                isGenerated: false
            });
